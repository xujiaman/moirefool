from attack_utils.network import U2NET
...
from attack_utils.options import opt
...
def generate_mask(input_image, net, palette, device = 'cpu'):
    ...
    # mask = np.resize(output_arr[0].astype(np.uint8)*255, (img_size[1], img_size[0]))
    mask = np.array(cloth_seg)
    return cloth_seg, np.expand_dims(mask, 0)
...
def main_generate_mask(img, seg_model, device):
    ...
    if img.mode == 'L':
        img = img.convert("RGB")
    cloth_seg, mask = generate_mask(img, net=model, palette=palette, device=device)

    return cloth_seg, mask
